...
scatter!(ax1, V1,markersize=35,color=[norm(v) for v in V1],colormap = :Spectral,strokewidth=2)

ax2 = Axis3(fig[1, 2], aspect = :data)
scatter!(ax2, V2,markersize=35,color=[norm(v) for v in V2],colormap = :Spectral,strokewidth=2)

ax3 = Axis3(fig[2, 1], aspect = :data)
scatter!(ax3, V3,markersize=35,color=[norm(v) for v in V3],colormap = :Spectral,strokewidth=2)

ax4 = Axis3(fig[2, 2], aspect = :data)
scatter!(ax4, V4,markersize=35,color=[norm(v) for v in V4],colormap = :Spectral,strokewidth=2)
