...
for testCase = 1:5
    if testCase == 1    
        ...
        cFun1(t) = 2.0 + 0.5.*sin(3*t)
        V1 = circlepoints(cFun1,np; dir=:acw)
        ...
        V2 = circlepoints(cFun1,np; dir=:acw)
    ...
    elseif testCase == 2
            ...
            cFun2(t) = 2.0 + 0.5.*sin(3*t)
            V1 = circlepoints(cFun2,np; dir=:acw)
            V1 = evenly_sample(V1, np)
            Q = RotXYZ(0.0,0.5*π,0.0) # Define a rotation tensor using Euler angles
            V1 = [(Q*v) .+ Vc[1] for v ∈ V1] # Rotate the coordinates
        
            V2 = circlepoints(cFun2,np; dir=:acw)
            V2 = evenly_sample(V2, np)
            Q = RotXYZ(0.0,0.5*π,0.0) # Define a rotation tensor using Euler angles
    ...
    elseif testCase == 3
        ...
        # Section 1
        cFun3(t) = 1.5 + 0.5.*sin(3*t)
        V1 = circlepoints(cFun3,np; dir=:cw)
        V1 = evenly_sample(V1, np)
        ...
        # Section 2
        cFun4(t) = 4 + 1.5*sin(5*t)
        V2 = circlepoints(cFun4,np; dir=:cw)
        V2 = evenly_sample(V2, np)
    ...
    elseif testCase == 4
        ...
        # Section 1
        cFun5(t) = rc/3 + rc/5 .* sin(3*t)
        V1 = circlepoints(cFun5,np; dir=:cw)
        V1 = evenly_sample(V1, np)
        Q = RotXYZ(0.5*π,0.0,0.0) # Define a rotation tensor using Euler angles
        V1 = [Q*v for v ∈ V1] # Rotate the coordinates
        ...
        # Section 2
        cFun6(t) = rc/4 + rc/6*sin(5*t)
        V2 = circlepoints(cFun6,np; dir=:cw)
        V2 = evenly_sample(V2, np)
        Q1 = RotXYZ(0.5*π,0.0,0.0) # Define a rotation tensor using Euler angles
        Q2 = RotXYZ(0.0,-0.25*π,0.0) # Define a rotation tensor using Euler angles
        Q = Q2*Q1
    ...
    elseif testCase == 5 
        ...
        # Section 1
        cFun7(t) = 3 + 0.25.*sin(3*t)
        V1 = circlepoints(cFun7,np; dir=:cw)
        V1 = evenly_sample(V1, np)
        V1_ori = deepcopy(V1)
        ...
        # Section 2
        cFun8(t) = 5 + 0.5*sin(5*t)
        V2 = circlepoints(cFun8,np; dir=:cw)
        V2 = evenly_sample(V2, np)    
        V2_ori = deepcopy(V2)
